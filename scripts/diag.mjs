...
await c.connect();
const EMAIL = "bong94688@gmail.com";
...
const nullday = ex.rows.filter(r => r.day_index === null).length;
console.log("day_index NULL 행 수:", nullday);
// 같은 (focus, exercise_id) 가 여러 일차에 같은 id 로 있는지(공유 행) 체크
const byId = {};
for (const row of ex.rows) { byId[row.id] = (byId[row.id]||0)+1; }
const dupIds = Object.entries(byId).filter(([,n]) => n>1);
console.log("중복 id(공유 행):", dupIds.length ? JSON.stringify(dupIds) : "없음");
await c.end();
