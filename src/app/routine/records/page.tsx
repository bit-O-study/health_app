...
export default async function RecordsPage() {
  ...
  return <main className="app-page app-container space-y-4">
    <header><p className="text-xs font-semibold text-emerald-600">운동</p><h1 className="mt-1 text-2xl font-bold">나의 운동 기록</h1><p className="mt-2 text-sm text-zinc-500">꾸준히 쌓아 온 변화를 확인하세요.</p></header>
    <div className="grid grid-cols-2 gap-3">
      <Link className="app-card p-5 font-bold text-emerald-700 dark:text-emerald-400" href="/settings/progress">성장 그래프 →</Link>
      <Link className="app-card p-5 font-bold text-emerald-700 dark:text-emerald-400" href="/settings/score">운동 점수 · 근육 밸런스 →</Link>
    </div>
    ...
    <Link href="/settings/history" className="block py-3 text-sm font-semibold text-emerald-700 dark:text-emerald-400">날짜별 상세 기록 →</Link>
  </main>;
}
