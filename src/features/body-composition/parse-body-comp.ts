...
export function parseBodyCompText(
  rawText: string,
  ...
  const skeletal = findByKeyword(blocks.full, /(골격근(?:량)?|SMM)/, {
    range: [5, 80],
  });
  if (skeletal !== null) out.skeletalMuscleKg = skeletal;

  const fatKg = findByKeyword(blocks.full, /체지방(?:량)?(?!률)/, {
    range: [0, 100],
  });
  ...
  const SEG: { ocr: OcrField; muscle: boolean; re: RegExp }[] = [
    {
      ocr: "muscleRightArm",
      muscle: true,
      re: /우(?:측)?\s*상지|오른\s*팔|R\s*arm/i,
    },
    {
      ocr: "muscleLeftArm",
      muscle: true,
      re: /좌(?:측)?\s*상지|왼\s*팔|L\s*arm/i,
    },
    { ocr: "muscleTrunk", muscle: true, re: /체\s*간|몸\s*통|trunk/i },
    {
      ocr: "muscleRightLeg",
      muscle: true,
      re: /우(?:측)?\s*하지|오른\s*다리|R\s*leg/i,
    },
    {
      ocr: "muscleLeftLeg",
      muscle: true,
      re: /좌(?:측)?\s*하지|왼\s*다리|L\s*leg/i,
    },
    {
      ocr: "fatRightArm",
      muscle: false,
