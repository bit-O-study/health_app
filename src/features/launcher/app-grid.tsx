...
export function AppGrid({ showCoach, searching = false }: { showCoach: boolean; searching?: boolean }) {
  ...
  const apps = launcherApps(showCoach).filter(app => app.name.includes(query.trim()));
  return <section aria-label="앱 런처" className="space-y-4">
    ...
    <div className="grid grid-cols-4 gap-x-2 gap-y-6 rounded-[24px] border border-zinc-200/80 bg-white px-3 py-6 dark:border-zinc-800 dark:bg-[#141c18]">
      {apps.map(app => { const Icon = APP_ICONS[app.icon]; return <Link key={app.id} href={app.href} prefetch={false} aria-label={app.name + " 앱 열기"} className="group flex min-h-20 flex-col items-center gap-2 rounded-xl focus-visible:outline-2 focus-visible:outline-emerald-600">
        <span className={"flex h-14 w-14 items-center justify-center rounded-[18px] bg-gradient-to-br text-white shadow-md transition-transform group-active:scale-95 " + app.color}><Icon aria-hidden="true" size={28} strokeWidth={1.7} /></span>
        <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-200">{app.name}</span>
      </Link>; })}
