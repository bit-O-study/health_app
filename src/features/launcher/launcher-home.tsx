...
export function LauncherHome({ dashboard, weekly, showCoach, searching, today }: {
  ...
  return <div className="space-y-6">
    ...
    <section aria-label="오늘의 요약" className="space-y-3">
      {cards.map(({href,label,Icon,color,title,detail}) => <Link key={label} href={href} aria-label={label + " 요약 열기"} className="group block rounded-[20px] border border-zinc-200/80 bg-white p-5 transition hover:border-emerald-400 dark:border-zinc-800 dark:bg-[#141c18]">
        ...
        {label === "식단" && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800"><div className="h-full rounded-full bg-emerald-600" style={{ width: Math.min(100, diet.targetKcal > 0 ? diet.eatenKcal / diet.targetKcal * 100 : 0) + "%" }} /></div>}
        {label === "캘린더" && <div className="mt-3 flex gap-2" aria-label="최근 7일 운동 기록">{contributions.filter(day => day.date <= today && day.level >= 0).slice(-7).map(day => <span key={day.date} title={day.date + " · " + day.minutes + "분"} className={"flex h-7 w-7 items-center justify-center rounded-lg text-[10px] " + (day.minutes > 0 ? "bg-emerald-600 text-white" : "bg-zinc-100 text-zinc-400 dark:bg-zinc-800")}>{new Date(day.date + "T00:00:00Z").getUTCDate()}</span>)}</div>}
      </Link>)}
    </section>
  </div>;
}
