 ...
 *
 * ⚠ 이 모듈은 **운동 카탈로그를 import 하지 않는다** — 세부근육 칩 하나 그리자고
 * 1,237개 목록(274 KiB)이 클라이언트 번들에 실리면 안 된다. 운동 id 만으로
 * 카탈로그를 뒤져야 하는 함수(`subMusclesForExercise` 등)는 `muscle-detail.ts` 에 있다.
 */
...
/**
 * 운동 → 특화 세부 근육 목록 (**이름·타깃을 이미 아는 호출부용**).
 *
 * 명시 매핑이 있으면 그것을, 없으면 이름·타깃 문구로 추론한다.
 * 카탈로그를 안 뒤지므로 목록 데이터 없이 동작한다 — 운동 객체를 이미 손에 쥔
 * 화면(픽커·검색 결과)은 이걸 쓴다. id 밖에 없으면 `muscle-detail.ts` 쪽을 쓴다.
 */
export function subMusclesForExerciseData(
  exerciseId: string,
  name: string,
  target: string,
): SubMuscle[] {
