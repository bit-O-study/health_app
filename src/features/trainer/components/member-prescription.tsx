...
export function MemberPrescription({ connectionId, memberId, memberName, exercises }: {
  ...
  return <section className="space-y-3" aria-labelledby="prescription-title">
    ...
    {exercises.length === 0 ? <p className="py-4 text-sm">회원이 운동을 등록하면 여기서 처방할 수 있어요.</p> : exercises.map(row =>
      <PrescriptionEditor key={`${row.id}:${row.updated_at}`} connectionId={connectionId} memberId={memberId} memberName={memberName} row={row} />)}
  </section>;
}
