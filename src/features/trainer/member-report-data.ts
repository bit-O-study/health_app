...
export async function getMemberReport(connectionId: string, memberId: string, from: string, to: string): Promise<MemberReportData | null> {
  if (!await getCurrentUser()) return null;
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.rpc("pt_member_detail", { p_link: connectionId, p_member: memberId, p_from: from, p_to: to });
...
export async function getMemberTodayPlan(
  ...
  if (!(await getCurrentUser())) return null;
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.rpc("pt_member_today_plan", {
    p_link: connectionId,
    p_member: memberId,
  });
