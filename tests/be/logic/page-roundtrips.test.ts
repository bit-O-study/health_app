 ...
 *
 * 서로 의존이 없는 조회를 `await` 로 하나씩 하면 그 왕복이 그대로 TTFB 에 쌓인다.
 * 지금은 함수 리전이 DB 와 같은 sin1 이라 왕복 1회가 ~5ms 로 싸졌지만, 그건 **리전이
 * 그대로일 때** 얘기다. 예전 서울(icn1) 배포에서는 같은 코드가 왕복당 70~90ms 였다 —
 * 순차 왕복은 리전 한 줄에 따라 언제든 다시 비싸진다.
 *
