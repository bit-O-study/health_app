...
const root=resolve(process.argv[2]??'tools/media/imports/exercise-video-20260925/wrist-curl-16');
const source=join(root,'tools/media/motion-guides/behind-the-back-wrist-curl.jpg');
const sha=createHash('sha256').update(readFileSync(source)).digest('hex').slice(0,12);
const cache=join(root,'tools/media/motion-guides/behind-the-back-wrist-curl/cut',sha);
const rows=[];
for(let n=1;n<=16;n++){
